    ...
    <div class="col-xs-12">
        <div class="box">
            <div class="box-header">
                <button class="layui-btn btn btn-layer" data-method="setTop">新建权限</button>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
...
<script>
    layui.use("table", function() {
        ...
        table.on("tool(info_filter)", function(obj) {
            var data = obj.data;
            if (obj.event === "edit") {
            console.log( '/admin/auth/edit/id/' + data.id);
                var that = this;
                //多窗口模式，层叠置顶
                window.editindex = layer.open({
                    ...
                    ,content: '/admin/auth/edit/id/' + data.id
                    ,btn: ['关闭']
                    ,yes: function() {
                ...
                console.log(obj)
                layer.confirm("确认删除权限" + data.name+ " ？", function(index) {
                    $.ajax({
                        url: '/admin/auth/delete/id/' + data.id,
                        type: 'post',
                        dataType: 'json',
                        data: {
